...
describe Api::V1::SocialsController, type: :controller do
  ...
  describe 'testing the routes mostly' do
    ...
    it 'should render the social index' do
      Social.create location_id: location.id

      get :index, format: :json, params: { location_id: location.slug, person_id: person.id }
      expect(response).to be_successful

      parsed_body = JSON.parse(response.body)
      expect(parsed_body['social'].length).to eq 1
    end

    context 'show action' do
      it 'should allow the user to view a person' do
        s = Social.create location_id: location.id
